...
p1 = ggplot(dataLongDailyTests %>% mutate(Value = ifelse(Variable == "New.deaths", Value*20, Value)) %>% filter(Variable %in% c("New.deaths","New.cases")))+
  ...
  annotate("label", x = as.Date(c("2020-03-16","2020-03-20","2020-03-29","2020-04-12","2020-05-04", "2020-05-18")), y = c(200, 300, 400, 500, 850, 950), label = c("Bar close","School close","StayHomeOrder","8pm data", "Curbside\npickup", "StaySafeOrder"))+
...
p2 = ggplot()+
  ...
  annotate("label", x = as.Date(c("2020-03-16","2020-03-20","2020-03-29","2020-04-12","2020-05-04","2020-05-18")), y = c(11, 15, 19, 23, 25, 31), label = c("Bar close","School close","StayHomeOrder","8pm data","Curbside\npickup", "StaySafeOrder"))+
  annotate("rect", xmin = as.Date(today(),format='%d-%B-%Y')-7, xmax = today(), ymin = 0, ymax = Inf, alpha = .15)+
...
p3 = ggplot(dataLongDailyTests %>% 
  ...
  geom_vline(xintercept = dataLongDailyTests %>% filter(Date > as.Date("2020-03-23")) %>% 
               distinct(Date) %>% 
               filter(Date %in% as.Date(c("2020-03-28","2020-04-12","2020-05-04", "2020-05-18"))) %>% 
               pull(Date), lty = 2, alpha = .4)+
  ...
  scale_y_continuous(sec.axis = sec_axis(~ ./max(dataLongDailyTests %>%
                                                   ...
                                                   pull(Value))*100, 		
                                         name = "Percentage (%)"))+
  annotate("label", x = as.Date(c("2020-03-28","2020-04-12","2020-05-04", "2020-05-18")), y = c(325, 375, 610, 610), label = c("StayHomeOrder","8pm data","Curbside\npickup", "StaySafeOrder"))+
  annotate("rect", xmin = as.Date(today(),format='%d-%B-%Y')-7, xmax = today(), ymin = 0, ymax = Inf, alpha = .15)+
  scale_x_date(date_breaks = "3 days", date_labels = "%b %d")+
  theme_minimal()+
