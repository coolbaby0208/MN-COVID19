...
vlineDf = tibble(Date = as.Date(c("2020-03-17","2020-03-18","2020-03-28","2020-05-04","2020-05-18", "2020-05-26","2020-05-27")),
                Label = c("Bar close","School close","Stay Home", "Curbside\npickup", "Stay Safe", "Unrest","25% worship"))
...
p1 = ggplot(dataLongDailyTests %>% mutate(Value = ifelse(Variable == "New.deaths", Value*secAxisConstant, Value)) %>% filter(Variable %in% c("New.deaths","New.cases")))+
  ...
  annotate("label", x = vlineDf$Date, y = c(200, 300, 400, 800, 800, 950, 880), label = vlineDf$Label)+
...
p2 = ggplot()+
  ...
  annotate("label", x = vlineDf$Date, y = c(11, 15, 19, 25, 25, 32, 29), label = vlineDf$Label)+
  annotate("rect", xmin = as.Date(today(),format='%d-%B-%Y')-7, xmax = today(), ymin = 0, ymax = Inf, alpha = .15)+
...
p3 = ggplot(dataLongDailyTests %>% 
  ...
  scale_y_continuous(sec.axis = sec_axis(~ ./max(dataLongDailyTests %>%
                                                   ...
                                                   pull(Value))*100, 		
                                         name = "Percentage (%)"))+
  annotate("label", x = vlineDf %>% filter(Date > as.Date("2020-03-23")) %>% pull(Date), y = c(325, 610, 610, 800, 710), label = vlineDf %>% filter(Date > as.Date("2020-03-23")) %>% pull(Label))+
  annotate("rect", xmin = as.Date(today(),format='%d-%B-%Y')-7, xmax = today(), ymin = 0, ymax = Inf, alpha = .15)+
  scale_x_date(date_breaks = "3 days", date_labels = "%b %d")+
  theme_minimal()+
