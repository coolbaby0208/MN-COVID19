...
* Testing
  ...
  + Total positive: **`r (dataWidePlain$Total.cases) %>% formatC(format="d", big.mark=",")`** (up `r (dataWidePlain$New.cases) %>% formatC(format="d", big.mark=",")`)
...
* Hospitalization   
  + Hospitalized as of today: **`r (dataWidePlain$Currently.hospitalized) %>% formatC(format="d", big.mark=",")`** (`r (ifelse(dataWidePlain$New.hospitalized <0, sprintf("down"), sprintf("up")))` `r (abs((dataWidePlain$New.hospitalized))) %>% formatC(format="d", big.mark=",")`)
    - Hospitalized in ICU as of today: **`r (dataWidePlain$ICU) %>% formatC(format="d", big.mark=",")`** (`r (ifelse(dataWidePlain$New.ICU <0, sprintf("down"), sprintf("up")))` `r (abs((dataWidePlain$New.ICU)))`)
...
## table 2
 dataWide %>%
   mutate(Date = format(Date, format = "%b %d")) %>% 
   filter(Date %in% tail(Date, 5)) %>%
   select(-starts_with("Total"), -starts_with("New"), -ends_with("Percent"), Daily.tests, Daily.tests.people) %>% 
   rename(`Hospitalized` = Currently.hospitalized, `Active` = Currently.sick, `ICU` = ICU, `Tests` = Daily.tests, `People tested` = Daily.tests.people) %>%
   select(Date, Hospitalized, ICU, Active, Tests, `People tested`) %>% 
   mutate_at(vars(Hospitalized:`People tested`), list(~formatC(., format="d", big.mark=","))) %>%
