...
Note: 
  - The csv data (current version: 2020-07-13) may not be updated regularly, but you can find the most recent data in the following link. The daily positve case number reported here is using today's total positve cases minus the the total positve cases from the previous day in the MDH Daily Update. So the postive case number may be slightly different from   the positve case count by specimen colletced date.   
